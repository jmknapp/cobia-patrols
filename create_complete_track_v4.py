#!/usr/bin/env python3
"""
Create patrol tracks centered to show full Pacific including Hawaii.
"""
...
def main():
    ...
    # Center map on Pacific, showing Hawaii to Asia
    # Use a center longitude of -170 (or 190 in wrapped coords) to show full Pacific
    m = folium.Map(location=[20, -170], zoom_start=3,
                   tiles='https://server.arcgisonline.com/ArcGIS/rest/services/Ocean/World_Ocean_Base/MapServer/tile/{z}/{y}/{x}',
                   attr='Esri Ocean',
                   world_copy_jump=True)  # Allow wrapping
    ...
    for patrol in range(1, 7):
        ...
        for i, seg in enumerate(segments):
            lons = [c[1] for c in seg]
            print(f"    Segment {i+1}: {len(seg)} pts, lon range [{min(lons):.0f}, {max(lons):.0f}]")
    ...
    print(f"\nSaved patrol_map.html (centered at lon=-170 to show Hawaii)")
